...
int ConvertText(int &symbol_count, char *&symbol_array)
{
    ...
    ifstream text_file("C:\\slova_clean.txt");
...
int Bigrams_without_Spaces_Crossing(int symbol_count, char *symbol_array)
{
    ...
    ofstream out;
    out.open("Bigrams_without_Spaces_Crossing.txt");
    if (out.is_open())
    {
        out << "Біграми без пробілів та з перетинами:" << '\n';
        for (int i = 0; i < k; i++)
        {
            for (int j = 0; j < 2; j++)
            {
...
// пошук біграм без пробілів та без перетинів
int Bigrams_without_Spaces(int symbol_count, char *symbol_array)
{
    int symbol_counter = symbol_count;
    ...
    if (out.is_open())
    {
        out << "Біграми без пробілів та без перетинів:" << '\n';
        for (int i = 0; i < k; i++)
        {
            for (int j = 0; j < 2; j++)
            {
